    ...
    public class GameClient : IDisposable
    {
        ...
        public void SendPacket(PointBlank.Core.Network.SendPacket bp)
        {
            try
            {
              ...
              '\t'
                    ...
                    if (array2.Length != 0)
                        this._client.BeginSend(array2, 0, array2.Length, SocketFlags.None, new AsyncCallback(this.SendCallback), _client);
                    bp.mstream.Close();
                    byteList.Clear();
                }
            }
        ...
        private void OnReceiveCallback(IAsyncResult ar)
        {
            try
            {
                ...
                Array.Copy(asyncState.buffer, 0, Buffer, 0, length);
                int FirstLength = BitConverter.ToUInt16(Buffer, 0) & short.MaxValue;
                byte[] numArray = new byte[FirstLength + 2];
                Array.Copy(Buffer, 2, numArray, 0, numArray.Length);
